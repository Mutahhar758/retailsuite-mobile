...
export default function ReceiptsListScreen() {
  ...
  const fetchReceipts = useCallback(async () => {
    try {
      const data = await receiptService.getList();
      setReceipts(data);
    } catch (error) {
  ...
  const renderItem = ({ item, index }: { item: ReceiptDto; index: number }) => (
    <Animated.View entering={FadeInUp.delay(index * 100).duration(400)}>
      <TouchableOpacity 
        style={styles.card} 
        onPress={() => router.push(`/receipts/${item.voucherNo}`)}
      >
        <View style={styles.cardHeader}>
          <Text style={styles.voucherNo}>RV-{String(item.voucherNo).padStart(5, '0')}</Text>
